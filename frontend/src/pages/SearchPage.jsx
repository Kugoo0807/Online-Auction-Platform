        ...
        {allProducts.length > 0 && (
            <div className="flex flex-col lg:flex-row gap-4 mb-8 bg-gray-50 p-4 rounded-xl border border-gray-100 items-start lg:items-center justify-between">
                ...
                <div className="flex flex-col sm:flex-row gap-3 w-full lg:w-auto border-t lg:border-t-0 pt-4 lg:pt-0 border-gray-200">
                    ...
                    <FilterDropdown label="Thứ tự" options={sortOptions} selectedValue={sortOrder} onSelect={(val) => { setSortOrder(val); setSearchParams({ keyword, page: 1 }); }} />
                    {(priceFilter !== 'ALL' || timeFilter !== 'ALL' || sortOrder !== 'default') && (
                        <button onClick={() => { setPriceFilter('ALL'); setTimeFilter('ALL'); setSortOrder('default'); setSearchParams({ keyword, page: 1 }); }} className="text-sm text-red-500 hover:underline font-medium ml-auto lg:ml-2 self-center whitespace-nowrap">Xóa bộ lọc</button>
                    )}
                </div>
            </div>
        )}
